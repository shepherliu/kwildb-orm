...
export const test = async () => {
	...
	const engine = NewEngine('test-db.kwil.xyz', 'https', 'test_cloud', privateKey, `7smx9dFXuLrbI8]ju+~:Jly#aMN,xis/`);
	...
	if(engine != null){
		...
		for(const i in moats){
			if(moats[i].moat === moat){
				const secret = await decryptKey(signature, address, moats[i].secret);
				const privateKey = await decryptKey(signature, address, moats[i].api_key);
		...
		console.log('show schemas');
		console.log(await engine.showSchemas());

		console.log('test create schema');
		console.log(await engine.createSchema('my_schema'));
		
		console.log('test create table');
		console.log(await engine.use('my_schema').createTable('test_cloud', {
		...
		console.log('is table exists');
		console.log(await engine.isTableExist('test_cloud'));		

		console.log('desc table');
		console.log(await engine.descTable('test_cloud'));

		console.log('test drop table');
		console.log(await engine.use('my_schema').dropTable('test_cloud'));		
		
		console.log('test drop schema');
		...
		console.log(await session.table('test_cloud3').truncate());

		console.log('insert table');
		await session.table('test_cloud3').insert([
			{
				id:2,
				name: 'ddd',
			},
